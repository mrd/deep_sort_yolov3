...
def main():
    ...
    parser.add_argument('--input-dir', help="input dir of video still frames", required=True)
    parser.add_argument('--output-dir', help="output dir of annotated frames", required=True)
    ...
    def copybackbuf(outdir, frameno):
        global no_lcd, no_framebuf, no_framebuf_device, COLOR_MODE
        ...
        filename = "{:06d}.jpg".format(frameno)
        cv2.imwrite(os.path.join(outdir,filename), outputframe)
   ...
   # Definition of the parameters
    ...
    try:
        while running:
            ...
            filename = inpfilenames.pop(0)
            frame = cv2.imread(os.path.join(args.input_dir,filename))
            ...
            copybackbuf(args.output_dir, frameno)
