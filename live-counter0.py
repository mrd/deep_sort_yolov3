...
def main():
   ...
   # deep_sort 
    ...
    tracker = Tracker(metric,max_age=5)
    delcount=0
    db = {}
        ...
        try:
            ...
            for _ in camera.capture_continuous(stream, format='jpeg', use_video_port=True):
                ...
                for track in tracker.tracks:
                    ...
                    if len(db[i]) > 1:
                        pts = np.array(db[i]).reshape((-1,1,2)) * ratios
                        draw.line(list(np.int32(pts).reshape(-1)), fill=(255,0,255), width=3)
                ...
                draw.text((0, image_height), str(delcount), fill=(0,0,255), font=FONT_LARGE)
